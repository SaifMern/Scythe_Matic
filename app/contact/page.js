...
const contactInfo = [
  ...
  },
  {
    icon: MapPin,
    title: 'Location',
    value: 'Office # 02,-05 2nd Floor, UBL Bank, Main Boulevard, Bahria Town, Lahore.',
...
const bottomContact = [
  ...
  },
  {
    icon: Phone,
    label: 'Phone:',
    value: '+92 316 166237612',
    href: 'tel:+923264859263',
  },
  {
    icon: MapPin,
    label: 'Address:',
...
function ContactCard({ item, index }) {
  ...
  return (
    <motion.a
      ...
      className={'group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl transition-all duration-500 hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.07] hover:shadow-xl hover:shadow-black/15'}
    >
      ...
      <div className="relative z-10 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mb-1 text-sm font-semibold text-primary">{item.value}</p>
...
function BottomCard({ item, index, isLast }) {
  const Icon = item.icon;
  return (
    <motion.a
      href={item.href}
      ...
      viewport={{ once: true, amount: 0.2 }}
      transition={{ duration: 0.6, delay: index * 0.08, ease: [0.22, 1, 0.36, 1] }}
      className={'group flex flex-col gap-4 rounded-2xl p-5 transition-all duration-500 hover:-translate-y-1 hover:bg-slate-50 sm:flex-row sm:items-start sm:p-6 ' + (!isLast ? 'lg:border-r lg:border-slate-200' : '')}
    >
      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-slate-950 text-primary shadow-lg transition-all duration-300 group-hover:bg-primary group-hover:text-white">
        <Icon className="h-6 w-6" strokeWidth={2.3} />
      </div>
      <div>
        <h3 className="mb-2 text-base font-bold tracking-tight text-slate-950 sm:text-lg">
          {item.label}
        </h3>
        ...
        <div className="container relative z-10 mx-auto">
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:gap-10">
            ...
            <motion.div
              ...
              className="space-y-4"
            >
              <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl sm:p-8">
            ...
            <motion.div
              ...
              className="relative"
            >
              <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl sm:p-8 lg:p-10">
      ...
      <section className="relative overflow-hidden bg-white px-4 py-10 text-slate-950 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
        ...
        <div className="container relative z-10 mx-auto">
          <motion.div
            ...
            whileInView="visible"
            viewport={{ once: true, amount: 0.2 }}
            className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-xl shadow-slate-950/5 sm:p-6 lg:grid-cols-3 lg:gap-0 lg:p-6"
          >
            {bottomContact.map((item, index) => (
              <BottomCard
                ...
                item={item}
                index={index}
                isLast={index === bottomContact.length - 1}
              />
            ))}
          </motion.div>
        </div>
